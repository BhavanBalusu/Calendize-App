    ...
    <div className="events-component">
      ...
      <div className="events-holder">
        {events.map((event, i) => <div className="event-wrapper" key={"wrapper " + i}>
          {(i == 0 || events[i].day.getDate() !== events[i - 1].day.getDate()) ? <p style={{ fontSize: '30px', width: '90%' }} className="date" key={"Date " + i} >{days[event.day.getDay()] + ", " + months[event.day.getMonth()].toLowerCase() + " " + event.day.getDate()} <hr className="dateDivider"></hr></p> : " "}<Event event={event} key={i} /></div>)
        }
      </div>
    </div>
